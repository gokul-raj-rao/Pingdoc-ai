...
def main():
    st.set_page_config(page_title='PingDoc')
    col1, col2 = st.columns([1, 4])  # Adjust the width ratio as needed

    with col1:
        st.image("logo.png", width=80)  # Adjust width as per your logo size

    with col2:
        st.title("PingDoc: AI Chatbot for Document Queries")
    
    
    with st.expander("Instructions to upload Text PDF/URL"):
        st.write("1. Pull up the side bar in top left corner.")
        st.write(
            "2. If uploading a PDF, click 'Upload PDF', select your file, and wait for 'Documents Loaded' confirmation.")
        st.write(
            "3. If entering a web URL, enter the URL, click 'Enter Web URL', and submit 'Process URL' and wait for 'Documents Loaded from URL' confirmation.")
        st.write(
            "4. After loading documents, click 'Create Vector Store' to process.Documents can only be uploaded once per session")
